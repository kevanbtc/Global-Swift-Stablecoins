...
/// @title ISO20022Emitter
/// @notice Emits typed events that reference ISO 20022 documents by content hash + URI.
///         Store the full XML/JSON off-chain (IPFS/S3); put the content hash + pointer here.
library ISO20022Emitter {
    event ISO20022Pacs009(
        bytes32 indexed docHash,
        string uri,
        uint64 msgId,
        uint64 txnTs,
        bytes32 indexed participantLEI
    );

    event ISO20022Camt053(
        bytes32 indexed stmtHash,
        string uri,
        uint64 asOfTs
    );
