...
contract PorBroadcaster {
    ...
    modifier onlyGov() { if (msg.sender != governor) revert CustomErrors.Unauthorized(); _; }
    ...
    function setGovernor(address g) public onlyGov { emit GovernorChanged(governor, g); governor = g; }
    function setPaused(bool p) public onlyGov { paused = p; emit Paused(p); }
    ...
    function quote(uint64 dstChain, address receiver) public view returns (uint256 feeWei) {
    ...
    function broadcast(uint64 dstChain, address receiver) public payable onlyGov returns (bytes32 id) {
        ...
        if (msg.value < feeQuote) revert CustomErrors.InvalidParam();
