...
/**
 * @title MerkleStreamDistributor
 * @notice Linear-vesting, gas-efficient distributor. One epoch = one merkle root.
 *         Each leaf holds {index, account, totalAmount}. Claim accrues linearly over [start,end].
 */
...
contract MerkleStreamDistributor is AccessControl {
    ...
    struct Epoch {
        ...
        uint64  start;
        uint64  end;
        uint256 total; // funding
        bool    active;
    }

    // epochId => index => claimed
    ...
    function vested(uint256 id, uint256 totalAmount) public view returns (uint256 v) {
        ...
        uint256 duration = e.end - e.start;
        v = (totalAmount * elapsed) / duration;
    }
    ...
    function claim(
        ...
        claimed[id][index] = allowed; // set to cumulative vested
