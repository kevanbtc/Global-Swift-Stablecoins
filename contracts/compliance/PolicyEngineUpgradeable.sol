...
contract PolicyEngineUpgradeable is Initializable, AC {
    ...
    function setCaps(uint256 _maxHolderBalance, uint256 _maxDailyOutflow) external onlyRole(GOVERNOR_ROLE) {
    ...
    function checkTransfer(Types.TransferContext calldata ctx) external view {
        if (globalPaused) revert Errors.GlobalPause();
        if (court.globalFreeze(ctx.token)) revert Errors.Frozen();

        if (registry.sanctioned(ctx.from) || registry.sanctioned(ctx.to)) revert Errors.Sanctioned();
        ...
        if (requireKYCApproved) {
            if (sf.kyc != Types.KYCState.APPROVED || st.kyc != Types.KYCState.APPROVED) revert Errors.AttestationInvalid();
        }
        if (sf.frozen || st.frozen) revert Errors.Frozen();
        if (block.timestamp < sf.lockupEnd) revert Errors.LockupActive();

        if (restrictJurisdictionUSOnly) {
            if (sf.juris != Types.Jurisdiction.US || st.juris != Types.Jurisdiction.US) revert Errors.JurisdictionBlocked();
        }
        // investor class example: disallow RETAIL as recipient
        if (st.klass == Types.InvestorClass.RESTRICTED) revert Errors.InvestorClassBlocked();
